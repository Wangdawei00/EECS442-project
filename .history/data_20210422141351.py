...
def loadZipToMem(zip_file, size):
    ...
    if size == "total":
        nyu2_train = list((row.split(',') for row in (csv['csv/nyu2_train.csv']).decode("utf-8").split('\n') if len(row) > 0))
        nyu2_valid = list((row.split(',') for row in (csv['csv/nyu2_valid.csv']).decode("utf-8").split('\n') if len(row) > 0))
        nyu2_test = list((row.split(',') for row in (csv['csv/nyu2_test.csv']).decode("utf-8").split('\n') if len(row) > 0))
    if size == "medium":
        nyu2_train = list((row.split(',') for row in (csv['csv/nyu2_train_m.csv']).decode("utf-8").split('\n') if len(row) > 0))
        nyu2_valid = list((row.split(',') for row in (csv['csv/nyu2_valid_m.csv']).decode("utf-8").split('\n') if len(row) > 0))
        nyu2_test = list((row.split(',') for row in (csv['csv/nyu2_test_m.csv']).decode("utf-8").split('\n') if len(row) > 0))
    if size == "small":
        nyu2_train = list((row.split(',') for row in (csv['csv/nyu2_train_s.csv']).decode("utf-8").split('\n') if len(row) > 0))
        nyu2_valid = list((row.split(',') for row in (csv['csv/nyu2_valid_s.csv']).decode("utf-8").split('\n') if len(row) > 0))
        nyu2_test = list((row.split(',') for row in (csv['csv/nyu2_test_s.csv']).decode("utf-8").split('\n') if len(row) > 0))
