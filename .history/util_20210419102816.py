...
def evaluate_model(model, loader, device):
    ...
    with torch.no_grad:
        ...
        for i, thebatch in enumerate(loader):
            batch = torch.Tensor(thebatch["image"]).to(device=device)
            truth = torch.Tensor(thebatch["depth"]).to(device=device)
            pred = model(batch)
...
def train_epoch(data_loader, model, criterion, optimizer, device):
    ...
    for i, batch in enumerate(data_loader):
        ...
        optimizer.zero_grad()
        X = torch.Tensor(batch["image"]).to(device=device)
        y = torch.Tensor(batch["depth"]).to(device=device)
        outputs = model(X)
        # calculate loss
        #loss = criterion(outputs, y)
        loss = DepthLoss(0.1)
        losses = loss(pred, truth).item()
