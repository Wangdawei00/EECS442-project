...
def loadZipToMem(zip_csv, zip_file, size):
    ...
    if size == "total":
        nyu2_train = list((row.split(',') for row in (csv['nyu_csv/nyu2_train.csv']).decode("utf-8").split('\n') if len(row) > 0))
        nyu2_valid = list((row.split(',') for row in (csv['nyu_csv/nyu2_valid.csv']).decode("utf-8").split('\n') if len(row) > 0))
        nyu2_test = list((row.split(',') for row in (csv['nyu_csv/nyu2_test.csv']).decode("utf-8").split('\n') if len(row) > 0))
    if size == "medium":
